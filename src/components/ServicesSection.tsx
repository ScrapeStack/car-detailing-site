...
export const ServicesSection: React.FC<ServicesSectionProps> = ({ onSelectServiceForQuote }) => {
  ...
  const getServiceIcon = (id: string) => {
    switch (id) {
      case 'complete-auto-detail':
        return <Sparkles className="w-5 h-5" />;
      case 'hand-wash-shine':
        return <Droplets className="w-5 h-5" />;
      case 'paint-correction':
        return <Wrench className="w-5 h-5" />;
      case 'ceramic-coating':
        return <Shield className="w-5 h-5" />;
      case 'headlight-tire-dressing':
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <p className="text-slate-400 text-sm max-w-md">
            Every vehicle serviced by Zephyr Auto Spa receives clinical grade attention, spot-free water, and master-level execution at 233 Brinsmade Ave in the Bronx.
          </p>
        </div>
              ...
              <div className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-black border border-white/15 shadow-2xl group">
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-[#090b10]/90 backdrop-blur-md p-3.5 rounded-xl border border-white/15">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-mono-tech text-amber-400 font-semibold uppercase">Zephyr Auto Spa Standard</span>
                    <span className="text-slate-400">Bronx, NY</span>
                  </div>
